...
public class BattleSystem : MonoBehaviour
{
    ...
    IEnumerator SetupBattle()
    {
        ...
        playerUnit.currentHP = PlayerController.vida;
    ...
    IEnumerator PlayerAttack(Element element)
    {
        if (state == BattleState.PLAYERTURN)
        {
            ...
            if (isDead)
            {
                ...
                PlayerController.vida = playerUnit.currentHP;
            }
            else
            {
    ...
    {
        if (state == BattleState.PLAYERTURN && PlayerController.potion >= 0)
        {
